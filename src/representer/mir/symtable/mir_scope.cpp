#include <cstddef>
#include <memory>

#include "representer/mir/symtable/mir_scope.hpp"


namespace avalon {
    /**
     * the constructor expects nothing
     */
    mir_scope::mir_scope() : m_parent(nullptr) {
    }

    /**
     * add_namespace
     */
    void mir_scope::add_namespace(const std::string& namespace_name) {
        m_namespaces.emplace(namespace_name, namespace_name);
    }

    /**
     * get_dtable
     * returns the symbol table of declarations found in this scope
     */
    mir_dtable& mir_scope::get_dtable() {
        return m_dtable;
    }

    /**
     * get_ctable
     * returns the symbol table of type constructors found in this scope
     */
    mir_ctable& mir_scope::get_ctable() {
        return m_ctable;
    }

    /*** TYPE DECLARATIONS ***/

    /**
     * add_type
     * add a new type declaration into this scope
     */
    void mir_scope::add_type(const std::string& ns_name, std::shared_ptr<mir_type>& type_decl) {
        m_dtable.insert_type(ns_name, type_decl);

        // we proceed to add constructors to the this scope as well
        // we import default constructors
        std::vector<mir_default_constructor> def_constructors = type_decl -> get_default_constructors();
        for(auto& def_constructor : def_constructors)
            add_default_constructor(ns_name, def_constructor);

        // we import record constructors
        std::vector<mir_record_constructor> rec_constructors = type_decl -> get_record_constructors();
        for(auto& rec_constructor : rec_constructors)
            add_record_constructor(ns_name, rec_constructor);
    }

    /**
     * get_type
     * given a type name, return the type that match them
     */
    std::shared_ptr<mir_type>& mir_scope::get_type(const std::string& ns_name, const std::string& type_name) {
        return m_dtable.get_type(ns_name, type_name);
    }

    /**
     * type_exists
     * given a type declaration, find if an existing type declaration matches it
     */
    bool mir_scope::type_exists(const std::string& ns_name, std::shared_ptr<mir_type>& type_decl) {
        return m_dtable.type_exists(ns_name, type_decl);
    }

    /**
     * type_exists
     * given a type name, find if an existing type declaration match the same
     */
    bool mir_scope::type_exists(const std::string& ns_name, const std::string& type_name) {
        return m_dtable.type_exists(ns_name, type_name);
    }

    /**
     * add_default_constructor
     * add a new default constructor into this symbol table
     */
    void mir_scope::add_default_constructor(const std::string& ns_name, mir_default_constructor& def_cons) {
        m_ctable.insert_default_constructor(ns_name, def_cons);
    }

    /**
     * add_record_constructor
     * add a new record constructor into this symbol table
     */
    void mir_scope::add_record_constructor(const std::string& ns_name, mir_record_constructor& rec_cons) {
        m_ctable.insert_record_constructor(ns_name, rec_cons);
    }

    /**
     * get_default_constructor
     * given a constructor name and arity, return the matching default constructor if it exists
     */
    mir_default_constructor& mir_scope::get_default_constructor(const std::string& ns_name, const std::string& cons_name, std::size_t arity) {
        return m_ctable.get_default_constructor(ns_name, cons_name, arity);
    }

    /**
     * get_record_constructor
     * given a constructor name and arity, return the matching record constructor if it exists
     */
    mir_record_constructor& mir_scope::get_record_constructor(const std::string& ns_name, const std::string& cons_name, std::size_t arity) {
        return m_ctable.get_record_constructor(ns_name, cons_name, arity);
    }

    /**
     * default_constructor_exists
     * given a constructor name and its arity, find if an existing default constructor matches the same
     */
    bool mir_scope::default_constructor_exists(const std::string& ns_name, const std::string& cons_name, std::size_t arity) {
        return m_ctable.default_constructor_exists(ns_name, cons_name, arity);
    }

    /**
     * record_constructor_exists
     * given a constructor name and its arity, find if an existing record constructor matches the same
     */
    bool mir_scope::record_constructor_exists(const std::string& ns_name, const std::string& cons_name, std::size_t arity) {
        return m_ctable.record_constructor_exists(ns_name, cons_name, arity);
    }

    
    /*** FUNCTION DECLARATIONS ***/

    /**
     * add_function
     * given a namespace and a function declaration, add the declaration to the scope
     */
    void mir_scope::add_function(const std::string& ns_name, std::shared_ptr<mir_function>& function_decl) {
        m_dtable.insert_function(ns_name, function_decl);
    }

    /**
     * get_function
     * given a namespace and a function name, return the matching function in that namespace with the same name
     */
    std::shared_ptr<mir_function>& mir_scope::get_function(const std::string& ns_name, const std::string& function_name) {
        return m_dtable.get_function(ns_name, function_name);
    }

    /**
     * function_exists
     * given a namespace and a function declaration, find if that function declaration already exists in that namespace
     */
    bool mir_scope::function_exists(const std::string& ns_name, std::shared_ptr<mir_function>& function_decl) {
        return m_dtable.function_exists(ns_name, function_decl);
    }

    /**
     * function_exists
     * given a namespace and a function name, find if the namespace contains a function with that name
     */
    bool mir_scope::function_exists(const std::string& ns_name, const std::string& function_name) {
        return m_dtable.function_exists(ns_name, function_name);
    }


    /*** VARIABLE DECLARATIONS ***/

    /**
     * add_variable
     * given a namespace and a variable declaration, add the later to the former
     */
    void mir_scope::add_variable(const std::string& ns_name, std::shared_ptr<mir_variable>& variable_decl) {
        m_dtable.insert_variable(ns_name, variable_decl);
    }

    /**
     * get_variable
     * given a namespace and a variable name, return the corresponding variable declaration if it exists in that namespace
     */
    std::shared_ptr<mir_variable>& mir_scope::get_variable(const std::string& ns_name, const std::string& variable_name) {
        return m_dtable.get_variable(ns_name, variable_name);
    }

    /**
     * variable_exists
     * given a namespace and a variable declaration, find if it already exists in this namespace
     */
    bool mir_scope::variable_exists(const std::string& ns_name, std::shared_ptr<mir_variable>& variable_decl) {
        return m_dtable.variable_exists(ns_name, variable_decl);
    }

    /**
     * variable_exists
     * given a namespace and a variable name, find if there is a variabl declaration for it in this namespace
     */
    bool mir_scope::variable_exists(const std::string& ns_name, const std::string& variable_name) {
        return m_dtable.variable_exists(ns_name, variable_name);
    }

    /**
     * set_parent
     * sets the parent of this scope
     */
    void mir_scope::set_parent(std::shared_ptr<mir_scope>& parent) {
        m_parent = parent;
    }

    /**
     * get_parent
     * returns the parent of this scope
     */
    std::shared_ptr<mir_scope>& mir_scope::get_parent() {
        return m_parent;
    }

    /**
     * has_parent
     * returns true if this scope has a parent scope
     */
    bool mir_scope::has_parent() {
        if(m_parent == nullptr)
            return false;
        else
            return true;
    }
}
